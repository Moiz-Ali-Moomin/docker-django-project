...
def cmd(para):
  ...
  print("-_-_-_-__-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-Clearing the screen_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-")
  time.sleep(2)
  os.system("clear")
...
while True:
 ...
 if int(choice) == 1:
  ...
  cmd("sudo docker-compose run python django-admin.py startproject {} .".format(name))
  
 elif int(choice) == 3:
  cmd("sudo docker-compose run python ./manage.py migrate")
  
 elif int(choice) == 4:
  cmd("sudo docker-compose run python ./manage.py createsuperuser")
  ...
  cmd("sudo docker-compose run python ./manage.py collectstatic")
 
 elif int(choice) == 7:
  dbname = input("Enter database username: ")
  dbpass = getpass.getpass(prompt='Enter you password: ')
  cmd("sudo docker exec -it docker_for_django_db_1 mysql -u {0} -p{1}".format(dbname, dbpass))
  
 elif int(choice) == 8:
  cmd("sudo docker-compose run python ./manage.py runserver")
